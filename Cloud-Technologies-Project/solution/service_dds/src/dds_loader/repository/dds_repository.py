    ...
    def s_user_names_insert(self,
        ...
        with self._db.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                        INSERT INTO dds.s_user_names (hk_user_names_pk, h_user_pk, username, userlogin, load_dt)
						VALUES(%(hk_pk)s, %(id_pk)s, %(username)s, %(userlogin)s, %(load_dt)s)
    ...
    def s_product_names_insert(self,
        ...
        with self._db.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                        INSERT INTO dds.s_product_names (hk_product_names_pk, h_product_pk, name, load_dt)
						VALUES(%(hk_pk)s, %(id_pk)s, %(name)s, %(load_dt)s)
    ...
    def s_restaurant_names_insert(self,
        ...
        with self._db.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                        INSERT INTO dds.s_restaurant_names (hk_restaurant_names_pk, h_restaurant_pk, name, load_dt)
						VALUES(%(hk_pk)s, %(id_pk)s, %(name)s, %(load_dt)s)
    ...
    def s_order_cost_insert(self,
        ...
        with self._db.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                        INSERT INTO dds.s_order_cost (hk_order_cost_pk, h_order_pk, cost, payment, load_dt)
						VALUES(%(hk_pk)s, %(id_pk)s, %(cost)s, %(payment)s, %(load_dt)s)
    ...
    def s_order_status_insert(self,
        ...
        with self._db.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                        INSERT INTO dds.s_order_status (hk_order_status_pk, h_order_pk, status, load_dt)
						VALUES(%(hk_pk)s, %(id_pk)s, %(status)s, %(load_dt)s)
						ON CONFLICT (h_order_pk) DO UPDATE
						SET h_order_pk = EXCLUDED.h_order_pk,
